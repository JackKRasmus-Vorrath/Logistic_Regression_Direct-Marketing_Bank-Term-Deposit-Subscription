...
# after dataframe, this reverts to numeric, so set it to factor, table flip--> (╯°□°）╯︵ ┻━┻
...
rfe_ctrl <- rfeControl(functions = caretFuncs, method = "repeatedcv", repeats = 5, verbose = TRUE)
...
y <- as.numeric(rfe_records$subscribed) # response

# run Recursive Feature Elimination
glmProfile <- rfe(x, y, rfeControl = rfe_ctrl
                  , sizes = c(1:number_predictors) # number of predictors that can be kept (candidate space)
                  , method="glmnet" # use glmnet
                  , family="binomial") # binary classification (i.e. logistic regression)

print(glmProfile) # show the results
