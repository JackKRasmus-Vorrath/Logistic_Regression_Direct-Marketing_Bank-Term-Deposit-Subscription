...
nloops<- 100   #number of CV loops
ntrains<-dim(dat.train.x)[1]  #No. of samples in training data set
cv.aucs<-c() # empty vector for later plotting of AUCs
...
for (i in 1:nloops){
  ...
  cvtrain.x<-as.matrix(dat.train.x[index,]) # convert to matrix
  cvtest.x<-as.matrix(dat.train.x[-index,]) # convert to matrix
  cvtrain.y<-dat.train.y[index] # isolate response sample
  cvtest.y<-dat.train.y[-index]
  
  cvfit <- cv.glmnet(cvtrain.x, cvtrain.y, family = "binomial", type.measure = "class") # create model using cross validation
  fit.pred <- predict(cvfit, newx = cvtest.x, type = "response") # predict y's using logreg model (creates matrix of probabilities corresponding test Xs)
  pred <- prediction(fit.pred[,1], cvtest.y) # creates 'ROCR' object containing binary-ized predictions (1 or 0) and other metadata
  ...
  cv.aucs[i]<-auc.train[[1]] #add y-values from auc.train for later plotting
  
}


hist(cv.aucs, breaks=60)
summary(cv.aucs)


## NEXT:  run model against the validation set
